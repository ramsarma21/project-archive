...
export const ASSETS: AssetRequirement[] = [
  ...
  { key: "bldg-warehouse-street", status: "EXISTING", path: "world/props/bldg-warehouse-street.glb", sizeM: [7, 12.4, 14], why: "The sugar house; retired from the run — superseded by the merchant's house (bldg-merchant) on the north row. Kept declared for reuse." },
  { key: "bldg-merchant", status: "EXISTING", path: "world/props/bldg-merchant.glb", sizeM: [9, 7.1, 15.2], why: "The merchant's house (Thomas) — the mission's SECOND enterable interior, generated the printshop way (build_civic_facade shell mode). A Georgian brick facade with a real open upper window on the south (no door), a solid parlour floor at 4.0, and a projecting window BALCONY that reaches 0.6 m south over the Shambles crate for the covert climb-in. Depth 15.2 (not 14): the balcony's south edge sits 7.6 m from the draw axis, so the box is that deep BOTH ways (a phantom north margin) to keep the draw centred on the collision — the safe corner-stud re-centre. Supersedes the sugar house; 9 m wide to clear the Town House scaffold approach." },
  { key: "billeting-pile", status: "EXISTING", path: "world/props/billeting-pile.glb", sizeM: [1.9, 0.86, 1.55], why: "1765 British-army quartering kit (stacked Brown Bess muskets, rolled bedrolls, leather knapsacks, a side-drum) — the billeting dressing that reads the merchant's parlour as occupied (concept-2 quartering stop). Meshy new-gen, scoped-optimized (714k->14k tris, textures 512). Non-blocking clutter kept clear of the 0.75 m path." },
