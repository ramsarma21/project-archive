...
const KNOWN_DEBT = new Map([
  ...
  ["DECK:WHARF_WAREHOUSE_A__ROOF", { category: "missing-or-short", rank: 3, band: 0.02, delta: -4.84, note: "MEASURED -0.61 / 15% at plane, far better than this recorded -4.84 (kept only as the never-exceed ceiling). The narrowing was TRIED 31-Jul and reverted; it needs an owner decision, not more measurement. wharf-a draws a shallow GABLE: a 1.78 m ridge cap at 5.35 (z 1.41..3.19, 14.8 m2) pitching to ~4.55/4.70 aprons, under a deck claiming the whole 10x8 footprint. Narrowing the deck to the drawn ridge is honest and BREAKS THE DESCENT AT BOTH ENDS, because a drop's take-off is the deck LIP and both lips move ~2.8 m inward: traversability then reports both WHARF_LEADS_SW->WHARF_DESC_ROOF and WHARF_DESC_ROOF->WHARF_DESC_MOUND_T landing on GROUND. Measured gaps: printshop lip z -2.50 -> ridge z 1.41 is 3.91 m on a 1.75 m drop, needing 6.55 m/s against RUN_SPEED 4.6 — unreachable by any run-off, only by a jump; ridge z 3.19 -> mound z 7.00 is 3.81 m on a 3.00 m drop, needing 4.87 m/s, and the mound cannot move north past z 6.00 without intersecting the warehouse. No stepped chain down the pitch is possible either: the mass fills the whole footprint to 5.35, so every standing spot below 5.35 inside x[-10,0] z[-2,6] is embedded in the blocker. So the fork is (a) keep the full-footprint deck and its ~0.95 m of invisible floor over the pitch, or (b) an asset that draws wharf-a's roof flat at its own top. Not authorable either way from this side." }],
  ["DECK:WHARF_WAREHOUSE_B__ROOF", { category: "missing-or-short", rank: 3, band: 0.02, delta: -1.37, note: "Re-pointed 31-Jul from the phantom 5.35 to the drawn roof; -1.05 -> -0.27 / 63% at plane, from 0%. CORRECTION to this note's first version, which blamed the residue on the deck's jetty: it is the RE-POINT'S OWN SIDE EFFECT. roofY sizes the placement box as well as the deck, so dropping it 5.35 -> 4.30 made height the binding axis of the contain-fit and shrank the mesh ~6%, carrying the very flat roof being chased from 4.30 down to 4.05 (measured off the placed GLB: 18.2 m2 at 4.05, 1.6 m2 at the 4.30 top). The move was still a big improvement and is worth keeping, but it cannot converge by iterating roofY — each re-point moves the target. GENERAL TRAP, and it is why wharf-a above is not authorable from this side either: structure() ties the mass top, the placement box and the walkable deck to ONE number, so a mesh whose principal walkable roof is not at its own top cannot be authored honestly. Closing this needs the asset to draw its roof at the top of its box." }],
  ["CLIMB_TO:CLIMBVOL_WHARF_ASC_2->WHARF_ASC_ROOF", { category: "missing-or-short", rank: 3, band: 0.00, delta: -1.88, note: "The mantle onto wharf-b's roof, re-pointed with the deck above to 4.30. Same residue and the same cause as DECK:WHARF_WAREHOUSE_B__ROOF — the contain-fit rescale, not the jetty." }],
