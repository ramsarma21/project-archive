// Variant of gen_prop_from_image.mjs for HERO assets (wharf hero brig):
// requests a higher Meshy target_polycount (60k, Bible §10 hero-ship budget)
// so masts/rigging survive; optimize_world_v3.py keeps hero ships at <=60k.
// Usage: node assets/pipeline/gen_prop_hero_from_image.mjs reference.png output.glb
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";
import { resolve, dirname, extname } from "node:path";

const [, , imageArg, outputArg] = process.argv;
if (!imageArg || !outputArg) {
  console.error("usage: node assets/pipeline/gen_prop_hero_from_image.mjs reference.png output.glb");
  process.exit(1);
}
