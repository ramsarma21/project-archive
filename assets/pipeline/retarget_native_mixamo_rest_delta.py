# Rebake native Mixamo motion clips onto a Mixamo-auto-rigged character whose
# skeleton rest pose is NOT the standard Mixamo T-pose reference (e.g. a rig
# fitted to an A-pose scan). Attaching the motion FBX actions directly (as
# bake_native_mixamo_character.py does) shifts every joint by the rest-pose
# difference: arms lift outward and palms turn up. This script transfers the
# CHANGE from rest instead, using the same rest-delta math as
# bake_character_anims.py:
#
#   world_rot_target(t) = ( world_rot_source(t) @ world_rot_source_rest^-1 )
#                         @ world_rot_target_rest
#
# Bones are matched by Mixamo name (FBX "mixamorig:X" -> glTF "mixamorigX").
# Bones the source clip does not drive (fingers) hold their rest offset.
#
# Run:
#   blender --background --python retarget_native_mixamo_rest_delta.py -- \
#     <character.glb> <out.glb> [clipsCsv]
import bpy
import os
import sys
...
# "absolute": drive target bone world rotations to equal the source's (correct
# when both rigs share Mixamo's joint axis conventions but have different rest
# poses, e.g. an auto-rigged A-pose scan playing standard-skeleton clips).
# "delta": transfer change-from-rest (correct when both rests are the same
# pose). Default absolute.
MODE = argv[3] if len(argv) > 3 else "absolute"

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
MIXAMO_DIR = os.path.join(ROOT, "source", "mixamo")
# Horizontal root motion belongs to the world controller (same set as
# bake_native_mixamo_character.py).
IN_PLACE = {"idle", "walk", "run", "carryWalk", "leftTurn", "rightTurn"}
...
# ---- Target character (glTF, bones named mixamorigX) ----
...
# Drop the actions imported with the character so rebaked clips can reuse the
# canonical names instead of colliding into "clip.001".
...
# Depending on the importer the ":" may or may not survive in bone names.
HIPS = "mixamorig:Hips" if "mixamorig:Hips" in tgt_rest_arm else "mixamorigHips"
assert HIPS in tgt_rest_arm, "target rig is not a Mixamo rig"
...
for clip in CLIPS:
    ...
    path = os.path.join(MIXAMO_DIR, filename + ".fbx")
    ...
    # FBX "mixamorig:X" -> target name (with or without the ":").
    name_map = {}
    ...
    # Scale source so hips rest heights match in world space (Blender Z-up).
    bpy.context.view_layer.update()
