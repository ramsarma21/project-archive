...
**This doc is the *worked instance* of `Day-Template.md`.** The template generalizes these beats into reusable archetypes + the design laws (the *why*) behind every decision here, so Days 2–4 replicate the intent rather than being re-perfected. Each beat below maps to a template archetype (B0→A0, B2→A2, … see Day-Template §3). When a Day-1 decision is made, its rationale is captured as a law in Day-Template §1 — edit both together.
