...
- Repo: pnpm monorepo. Layout after the refactor:
  ...
  - `apps/api` — Fastify + Postgres: auth, event-sourced save/replay via chapter registry, open-response grading (TrueFoundry `gemini-group/gemini-3.5-flash-lite`, server-side only, on the single shared `TRUEFOUNDRY_API_KEY`). Three claims in this line were stale and are corrected: the model was re-picked off `nova-micro` on a measured 15% false-negative rate; grading is no longer envelope-encrypted (that module and its tables were dropped in migration `009`); and it is no longer gated on a dedicated secret.
