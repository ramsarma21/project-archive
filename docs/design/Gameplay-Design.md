...
**Amendment, 25 July 2026:** **L-G** (§2), the guardrails checklist (§14), and §15 amended to absorb the game restructure — integer Ranks derived from Level, stat axes and ability verbs and Titles cut, one difficulty for everyone, mandatory 3-minute learning modules that pay nothing, three mission attempts with decaying XP, zero in-mission knowledge checks, and PvP as the only leaderboard. Derives from `docs/chapters/boston-1765/Mission-Slate.md` §1 and `.cursor/plans/pvp_and_mastery_redesign_aeaa8e2b.plan.md`. **The rest of this doc still describes the superseded open-district Act container and is stale on that axis; the specific unreconciled sections are listed at the end of §15.**
...
**L-G. The non-goals hold, as amended** *(re-amended 25 July 2026)*. Learning-coupled progression — XP and Levels, an integer **Rank** advancing one step per ten Levels, abilities unlocked at Level milestones, the Codex — and a bounded social layer (friends-by-code, opt-in 1v1 PvP) are **permitted**, but only under the seven guardrails in **PRD §17.1**, which bind this doc in full: every reward stays coupled to demonstrated learning or performance, carrot only, and **the only leaderboard is PvP**, which ranks duel outcomes and **never** the learning record. Cut and not returning: the lettered E→S ladder, stat axes, the fixed named set of ability verbs, Titles, and every PvE competitive surface — leaderboards, flawless-run tracking, ghosts, times, and scores. Still forbidden outright: crafting, inventory grind, morality meters, streaks, endless-engagement loops, reward-for-time-spent, and any reward decoupled from learning or performance (PRD §17). There is **one difficulty for everyone** — no bands, no Easy mode, no per-player easing — and there was never a knowledge dial (PRD §17.2). Fun still comes from *doing believable things well in a believable world* and from *meaningful choice* — the progression layer reports and rewards that, it never substitutes for it. The press-pull model (mastery expressed as relationship + craft quality, never a score) stays the template for in-world feedback.
...
- Progression and the social layer obey the **seven guardrails in PRD §17.1**; still no **crafting / inventory grind / morality meter / streaks / endless-engagement loops / reward-for-time-spent** (PRD §17, L-G).
- **The only leaderboard is PvP**, and it ranks duel outcomes only. No PvE leaderboard, no flawless-run tracking, and never a public ordering of mastery, concept coverage, assessment results, or answer accuracy (PRD §17.1, amended 25 July 2026).
- **One difficulty for everyone** — no bands, no Easy mode, no per-player easing — and **no knowledge dial** (PRD §17.2).
- **Required learning is uncapped.** The mandatory 3-minute module and the mastery assessment retry without limit; the three-attempt cap belongs to missions, which carry no knowledge checks and are not required learning (PRD §17.1, FR-6).
- Formal assessment is **route-independent**, **private**, and **never an official STAAR score**; mission attempt history, Rank, and PvP standing never enter it, and duel verdicts are gameplay results rather than assessment evidence (FR-11).
...
- **PRODUCT-REQUIREMENTS §17 (non-goals) — amended 24 July 2026.** The PRD permitted learning-coupled progression and a bounded async social layer, each bound by the guardrails in **PRD §17.1**, and separated adaptive *mechanical* difficulty from the still-forbidden knowledge dial in **PRD §17.2**. Its enumeration of that progression (Rank E→S, stat axes, ability verbs, Titles) is superseded by the 25 July amendment below.
- **PRODUCT-REQUIREMENTS §17 / §17.1 / §17.2 — re-amended 25 July 2026.** The game restructure lands in the PRD as follows, and **L-G** (§2) and the guardrails checklist (§14) are updated to match.
  - **Progression is one number and one derived number.** Level starts at 0, is driven only by mission XP, and resets each chapter; **Rank is an integer, Level divided by ten**, and carries across chapters. The lettered ladder, stat axes, the fixed named set of ability verbs, and Titles are cut. Abilities remain, unlock at Level milestones, are chapter-scoped in PvE, and accumulate permanently in the PvP loadout. **Magic and fantastical abilities are now allowed**, justified by the time-traveler fiction — the runner is the one thing in the scene that does not belong to the era. The world, the events, the documents, and every historical claim stay accurate; that boundary is narrow and firm.
  - **PvE has no competitive layer.** PvE leaderboards and flawless-run tracking are cut, and **the only leaderboard is PvP**. That ladder strains the original prohibition on ranking learners by what they know, because duel questions are free-response history and correct answers convert into bullets. The PRD relaxes one part of the prohibition and retains the other absolutely; the reasoning and the seven mitigations are recorded in the amendment block at the end of **PRD §17.1** and must be read before anything touches the ladder.
  - **One difficulty for everyone.** Easy mode, the five bands, mission levels, the player-minus-mission delta, and all adaptive or hidden per-player easing are cut, so **PRD §17.2** no longer describes two axes — there is no difficulty dial at all. The knowledge bar was never adjustable and still is not; the guardrail now holds by construction rather than by policing.
  - **Learning modules are mandatory and pay nothing.** Always exactly 3 minutes, never 90 seconds, required before a first mission attempt and again before every retry, and worth zero XP. Recorded in **PRD FR-6**.
  - **Missions carry three attempts with decaying XP** — full, two-thirds, one-third, then permanent failure at zero — and contain **zero knowledge checks**. All knowledge assessment moved into a six-round boss duel of free-response questions, where a correct answer grants 3 bullets and a wrong one grants 1. The "retryable without limit" guarantee in **PRD §17.1** survives intact because missions are explicitly not required learning; the module and the mastery assessment are, and both stay uncapped.
  - Derives from `docs/chapters/boston-1765/Mission-Slate.md` §1 and `.cursor/plans/pvp_and_mastery_redesign_aeaa8e2b.plan.md`. `Game-Concept-The-System.md` is superseded and now carries a staleness banner.
- **PRODUCT-REQUIREMENTS §7 (product structure):** the "Mission Day / End Day" unit is reframed to **Act / checkpoint** for Boston. The atomic-commit, resume, and assessment *mechanics* are unchanged; only the container name and boundary placement change. A PRD revision should record: Chapter → 4 Acts → 5 checkpoints (Archive year-transitions), with invisible autosave continuing within each Act. **Overtaken 25 July 2026:** the container is now the five-minute instanced mission, so do not make this PRD revision as written; the §7 rewrite waits on the full reconcile pass below.
...
- **New authoring workstreams:** the curated micro-concept set per Act (§3), the TEKS-tagged STAAR question bank (§4), the side-job content (§9), and the stealth system engineering pass (§7) each need their own detailed pass. Add the **duel question bank** — free-response items carrying question, acceptable answers, rubric, and associated Codex cards, six consumed per attempt and deep enough that a retry never repeats. It is separate from and additional to the multiple-choice STAAR bank, and neither substitutes for the other.
- **Unreconciled by the 25 July amendment, and known stale in this doc.** The restructure replaces the open-district Act with a hub launching **five-minute instanced solo missions** — 3:00 of continuous parkour and stealth, then a 2:00 six-round duel. Everything here that assumes the older container is stale until a full pass lands: the four-Act / five-checkpoint structure (§1), the 25–30 minute Act rhythm and its Sync-spacing and breather rules (§11), the Act 1 segment map (§11A), in-Act Archive Syncs and NPC dialogue checks as the mission learning layer (§3, §4, §8, §11A), and the non-violence framing in §7, which the settled decision that every mission ends in a **gun duel** contradicts head-on. The world systems — stealth, suspicion, heat, chase, stamina, traversal, and compound-verb skill — stay useful as arena-level design; their container does not. **One exception is deliberately left open rather than cut:** L-G removes stat axes, but **Standing** (§8) is the only one of the five stats implemented and working, and whether it survives as a stealth and dialogue input is an unresolved owner decision (`standing-survival-decision` in the plan). Treat §8's Standing material as pending that call, not as already dead. The full rewrite is tracked as `doc-reconcile` in `.cursor/plans/pvp_and_mastery_redesign_aeaa8e2b.plan.md`.
