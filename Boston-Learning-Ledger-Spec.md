...
- **Written by:** every tracked surface — Found-History focus-reads (`Act-1-Environmental-Lore.md` §4), mechanic completions (`Boston-Mechanics-Spec.md`), NPC/side-job finishes, the fixed event. The `sourceId` is the same id used in `Act-1-World-Content.md`, so provenance is a stable cross-reference.
...
- **Scale target (Act 1 / Boston):** 3 macros + the Boston gated facts + the pattern SEs + ~14 micros. The full 23-SE ownership and `ONCE`/`SPIRAL` tags come from `Boston-STAAR-Coverage-Map.md` / `Curriculum-World-Map.md` — this registry is where those tags become runtime-legible.
...
- **Locked decisions (2026-07-21):**
  ...
  1. **Default `ConceptClass` assignment** (SME to ratify final SE codes, not a systems blocker):
     ...
     - `MICRO` = the 14 enrichment concepts in `Act-1-Micro-Concepts.md`.
